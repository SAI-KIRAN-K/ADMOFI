...
public class CustomAdaptervunglei extends CustomAdapterImpl {
    ...
    private AdmofiView mAdmofiView = null;
    ...
    private void loadInterstitialV(Context context, AdmofiAd mAdCurrent) {
        ...
        try{
            AdmofiUtil.logMessage(null, Log.DEBUG, "Admofi Vunglei LoadInter");
    ...
    private final VungleAdEventListener vungleListener = new VungleAdEventListener() {
        ...
        public void onAdEnd(String s, boolean b, boolean b1) {
            AdmofiUtil.logMessage(null, Log.DEBUG, "Admofi Vunglei Event Completed:::"+b+":::"+b1);
            stopTimer();
            isActive = false;
        ...
        public void onAdStart(String s) {
            AdmofiUtil.logMessage(null, Log.DEBUG, "Admofi Vunglei Event ad started");
        }

        @Override
        public void onUnableToPlayAd(String s, String s1) {
            AdmofiUtil.logMessage(null, Log.DEBUG, "Admofi Vunglei Event Failed");
            if (isActive) {
                isActive = false;
                stopTimer();
        ...
        public void onAdAvailabilityUpdate(String s, boolean b) {
            if (isActive) {
                AdmofiUtil.logMessage(null, Log.DEBUG, "Admofi Vunglei Event Ready");
                isActive = false;
                stopTimer();
                adEventReady(null);
            }
    ...
    public boolean showinterstitial() {
        try{
            AdmofiUtil.logMessage(null, Log.DEBUG, "Admofi Vunglei Show Inter");
            ...
            if (getAd().getAdType() == AdmofiConstants.AD_TYPE_INTERSTITIAL) {
                try {
                    if ((vunglePub != null) && (vunglePub.isAdPlayable(PLACEMENT_ID))) {
                        ...
                        overrideConfig.setIncentivizedUserId("vungle_x_" + mAdmofiView.getUniqId() + "_x_" + mAdmofiView.getWalletInstance().getAppId() + "_x_" + AdmofiUtil.getApplicationPackage(mContext));
                        overrideConfig.setSoundEnabled(true);
                        overrideConfig.setIncentivizedCancelDialogTitle("close The video ?");
                        overrideConfig.setIncentivizedCancelDialogBodyText("you will loose the money");
