...
public class CustomAdaptersupersonicvi extends CustomAdapterImpl {
	...
	private void loadInterstitial(Context context,final AdmofiAd mAdCurrent) {
		try {
			AdmofiUtil.logMessage(null, Log.DEBUG, "Admofi supersonicvi Event Load Interstitial");

			if(!advertisingId.equalsIgnoreCase(CustomAdaptersupersonicvi.ADVT_ID)){
			...
			AdmofiUtil.logMessage(null, Log.DEBUG, "Admofi supersonicvi Event Load Interstitial Exception :: "+e.getLocalizedMessage());
			adEventLoadFailed(AdmofiConstants.ADM_TPFAILED_EXCEPTION);
		}
	}
	...
	private void requestInterstitial(String apikey,String advId){
		try{
			...
			if(CustomAdaptersupersonicvi.rewardedVideoInitSuccess){
				...
				CustomAdaptersupersonicvi.rewardedVideoInitSuccess = true;
	...
	public void stopTimer() {
		if (null != timer) {
			try {
				AdmofiUtil.logMessage(null, Log.DEBUG,"Admofi supersonicvi Stop Timer");
				timer.cancel();
				timer.purge();
			} catch (Exception e) {
				e.printStackTrace();
	...
	public boolean showinterstitial() {
		try {
			...
			if((getAd().getAdType() == AdmofiConstants.AD_TYPE_INTERSTITIAL) &&(IronSource.isRewardedVideoAvailable())) {
				AdmofiUtil.logMessage(null, Log.DEBUG, "Admofi supersonicvi Event showInterstitial");
	...
	RewardedVideoListener rewardListener = new RewardedVideoListener() {
		...
		public void onRewardedVideoAdRewarded(Placement placement) {
			try {
				...
				adEventRewardSuccess(new AdmofiReward("Admofi supersonicvi Points "+rewardName, rewardAmount ,true,"supersonicvideo Reward Success"));
